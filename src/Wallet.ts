...
class Wallet extends RpcClient {
    ...
    public async addKey(mnemonic: string[], type: string = 'new_wallet'): Promise<AddKeyResponse> {
    ...
    public async getTransactions(walletId: string): Promise<Transaction[]> {
        const { transactions } = await this.request<TransactionsResponse>('get_transactions', { wallet_id: walletId });

        return transactions;
    }

    public async getNextAddress(walletId: string): Promise<string> {
        const { address } = await this.request<NextAddressResponse>('get_next_address', { wallet_id: walletId });
